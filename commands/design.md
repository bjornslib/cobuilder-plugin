---
title: "cobuilder-architect: Design"
status: active
type: command
...
off the timeline until submit mode files it under a real pull request
number.
...
Invoke the `odyssey` skill in design mode, forwarding any arguments the
user supplied after `/cobuilder-architect:design` (`--non-interactive`):

```
Skill("odyssey", args="design $ARGUMENTS")
```
...
```
/cobuilder-architect:design
/cobuilder-architect:design --non-interactive
```
