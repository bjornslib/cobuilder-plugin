...
files already written by `/cobuilder-architect:baseline` / `/cobuilder-architect:review`
in the `cobuilder-pr` plugin. If no bundle exists yet, the skill will tell
you to run `/cobuilder-architect:baseline` first rather than starting a
server against nothing.
