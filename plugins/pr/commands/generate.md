...
The captured intent stays in `.cobuilder-architect/`, so `/pr:review` later reads
what the author said instead of inferring it.

Invoke the `odyssey` skill in generate mode, forwarding any arguments the user
supplied after `/pr:generate` (`--repo <path>`, `--store local|central`,
...
/pr:generate
/pr:generate --draft
/pr:generate --no-create
/pr:generate --base develop
/pr:generate --prs 73
/pr:generate --prs 73 --stage post
/pr:generate --repo ~/code/other-project --prs 12
```
