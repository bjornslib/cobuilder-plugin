...
from _bundle_meta import SCHEMA_VERSION, read_plugin_version, require_compatible, stamp_generator
...
def load_index(bundle_dir: Path) -> dict:
    """Return the window.INDEX object, or {} if index.json / index.js is missing.
    A PR artifact may carry an empty index map. Never fail publish because
    the file is absent."""
    json_path = bundle_dir / "data" / "index.json"
    if json_path.exists():
        try:
            return json.loads(json_path.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    js_path = bundle_dir / "data" / "index.js"
...
window.INDEX = {escape_script_close(json.dumps(index_obj, ensure_ascii=False))};
...
def render_for_pr(
    ...
    index_obj = load_index(bundle_dir)
...
def compute_source_hash(entry: dict, adrs_obj: dict, diffs_js: str | None, diagrams_by_level: dict[str, str]) -> str:
    ...
    # `/artifact:publish` will re-export all of them once, even ones with no
...
def main() -> None:
    ...
    require_compatible(bundle_dir, PLUGIN_NAME)
    ...
    for pr_num in pr_nums:
        ...
        if entry is None:
            print(
                ...
                f"remediation: run /pr:generate --prs {pr_num} first.",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        stamp_generator(bundle_dir, PLUGIN_NAME, read_plugin_version())
