...
- `--repo <path>` selects that repo's bundle directly, no prompt. If that
  ...
  `/pr:baseline` rather than finding a stale-looking bundle.
...
files already written by `/pr:baseline` / `/pr:review`
in the `pr` plugin. If no bundle exists yet, the skill will tell
you to run `/pr:baseline` first rather than starting a
