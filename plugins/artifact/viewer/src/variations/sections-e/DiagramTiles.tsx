 ...
 *
 * THE RUNTIME LOADS WHEN THIS SECTION MOUNTS. The old rule, recorded in `Diagram.tsx`
 * and in interaction-design section 2.3, was that Mermaid loads on the reader's press
 * and at no other moment, because the runtime is large. A tile cannot wait for a press:
 * the tile IS the drawing. So the rule becomes: the runtime loads when the reader
 * reaches the Diagrams section, and at no other moment. No other level, and nothing
 * before this section mounts, pays for it.
 *
 ...
 * which is the only inner-HTML write in this variation.
 */
...
import type { MermaidApi, Theme } from "./Diagram";
import { loadMermaid } from "./Diagram";
 ...
 */
function uncap(svg: string): string {
