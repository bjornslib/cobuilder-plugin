 ...
 * work item against another before opening either. So this file carries a three-state
 * readiness — `present`, `partial`, `absent` — and the board is its only reader.
 *
 * The six kind names, the three readiness words, and the rules below are the record
 * mosaic's, copied here for the reason `atoms.tsx` gives for copying its card
 * vocabulary: every directory under `variations/` stays independently removable while
 * the arrangement is under review, and a cross-variation import would break that.
 *
...
import type { DesignRecord } from "./records";
 ...
 */
function narrativeLevel(record: DesignRecord, key: string): { narration?: string } | null {
  const narrative = record.narrative as Record<string, unknown> | undefined;
  const source = (narrative?.levels ?? narrative) as Record<string, unknown> | undefined;
 ...
 */
function goalVerdict(goal: DesignRecord["goal"] | undefined): Verdict {
  ...
  if (goal.min_work?.challenge_stage_run === false) missing.push("design mode's stages 2 to 7");
