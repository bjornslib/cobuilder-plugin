...
description: >
  ...
  `/cobuilder-architect:baseline`, `/cobuilder-architect:review`, `/cobuilder-architect:view`,
  `/cobuilder-architect:publish`, or `/cobuilder-architect:generate`.
---
...
1. An explicit `--repo <path>` argument forwarded by the command. This can
   ...
   (for example `/cobuilder-architect:review --repo ~/code/other-project --prs 12`).
...
View mode and Publish mode moved to the `cobuilder-artifact` plugin.
...
11. **Offer to continue into narrative generation.** Ask the author
    ...
    `/cobuilder-architect:review --prs <N>`.
...
`/cobuilder-architect:generate`, once the PR exists, files the content
into `story.json`.
