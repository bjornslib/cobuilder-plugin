...
the PR's `assessment` block in `data/story.json`. Generate mode runs this
after the interview in `interview-guide.md`, and before it opens the pull
request.
...
The tier sets how deep the assessment goes, and generate mode reports it.
...
A `rework` verdict does not stop generate mode from opening the pull request.
The author reads the verdict, and the author decides. See the Generate mode
section of `SKILL.md`.
...
- **No new ADR.** A pre-merge decision is a proposal. Review mode writes the
