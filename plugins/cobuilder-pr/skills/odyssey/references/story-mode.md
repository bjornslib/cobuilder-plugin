...
(`<bundle-dir>/data/story.json`). One artifact serves two readers. A PM skims
...
to `narration`, `problem`/`solution`, `beats`, and `voice` alike — the
register (plain vs. developer-precise) stays the same, only the
tense/certainty changes.

**When the PR carries an `intent` block, read it first.** Generate mode
(`/cobuilder-architect:generate`) interviews the author before the PR opens and writes
...
- `intent.out_of_scope` is the honest boundary of the change, which a diff
  ...
  surprising absence.
- `intent.unknowns` is not narrative material. It is a review signal. It
  lives in the assessment, and it does not belong in a story a reader
  trusts.

Still ground every *mechanism* claim in the diff. The author states the
intent. Only the code states what the change actually does, and the two
can disagree. That disagreement is exactly what generate mode's post stage
reports. That is a later, post-merge check. `interview-guide.md` §3a's
self-consistency check runs earlier, before the `intent` block is ever
written.

An `intent` block whose `source` is `"inferred"` was not stated by anyone.
Treat it as one more reading of the evidence, at the same weight as your
own.

## 1. Framework origin

The four-part structure comes from Geoffrey Litt's **explain-diff**
framework ("Understanding is the new bottleneck"), which explains a merged
PR as **Background → Intuition → Code → Quiz**. Background gives a
newcomer enough system context to place the change. Intuition distills
the change to its essence with one concrete example. Code walks the
actual diff, grouped by intent rather than file order. Quiz checks
whether the explanation actually transferred understanding. Story mode
adapts the first three parts to the bundle's four-level schema — see §2.
The quiz is not implemented (§5).
...
Level 1 is the entry a PM sees first in the timeline scrubber — keep it to
a single sentence a non-engineer can parse, plus the raw size numbers.
Level 2 is where most of the explain-diff prose lives: `problem` and
`solution` are precise, with code identifiers in backticks, and
`narration` restates the same content in plain language. Level 3 exists
only when the PR carries real architectural weight. Pull `alternatives`
and `forces` straight from the PR's ADRs (`<bundle-dir>/data/adrs.json`,
records with matching `source_pr`, extracted in the same sweep — see
`decision-records-lite.md`). Do not invent options the ADR did not
record. Level 4 is the only level allowed to name files — group them by
shared intent (e.g. "DDD rename & feedback sidecar," "Pi SDK extension"),
3-6 files per group, with a one-sentence `note` on why the group is one
unit of change.

`verify_bundle.py` requires a non-empty `narration` for level 3 on every
PR, with no exception for a PR that carries no structural decision. So
"exists only when the PR carries real architectural weight" is a
statement about the *content*, not about whether the field gets written.
For a PR with zero ADRs (`adrs: []`, per `decision-records-lite.md`
§3.3's "most PRs produce zero or one" rule), the expected level-3 content
is an honest, one- or two-sentence null-decision statement. State plainly
that the PR touches no module boundary, dependency direction, or public
interface, and name what kind of change it actually is (docs, style,
config, test-only, etc.), instead of inventing forces or alternatives to
fill the field. That statement satisfies both this reference's "only when
it carries weight" discipline and `verify_bundle.py`'s unconditional
non-empty check. It is the correct content for a decision-free PR, not a
workaround.
...
Contract / Boundary cards. Those fields feed other cards on the same
screen: The Problem / The Solution on level 2, and the ADR sheet's own
Forces/Decision on level 3, opened by clicking an ADR badge. The beat
cards themselves read one field: `levels.<level>.beats`, present on
**both** `problem_solution`
...
- **Level 2 `beats`**: at least one `background` item (the deep→narrow
  ...
  toy example from §3). This is the same content already going into
...
- Every level entry that has both a `narration` and a detail-register field
  ...
  them (e.g. do not put a fact only in `detail` that `narration` needs to
  make sense).
...
- Spoken prose only. No backticks, no markdown emphasis, no bullet lists —
  ...
  meant as formatting.
- Spell out symbols a voice cannot pronounce cleanly: `dot-githooks` not
  `.githooks`, `bridge dot py` not `bridge.py`, "no-verify" as words, not a
  flag rendered in code font.
- Write numbers as words where that is how a person would say them in
  conversation ("the eleventh of July," "under a second"). Code-adjacent
  numbers that are read as tokens (ADR-0008, PR seventy-three) can stay
  closer to their written form, since that is how they are actually said
  aloud.
