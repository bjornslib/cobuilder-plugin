...
approved_by: ""
problem: "Accepting a path commits a team to merging ten pull requests in a stated order. An execution onto the default branch makes the first merge irreversible and the whole path unabandonable. It also gives the reviewer no tree to inspect between one bundle and the next. It makes the runner the first thing in this plugin family that writes to a shared branch."
decision: "A runner reads an accepted path from the ledger. It merges each bundle onto its own integration branch, built from the previous bundle's branch and rooted at the common ancestor. The default branch receives one pull request at the end, from the last bundle's branch. The reviewer commits to the path once, and separately chooses the pace: all bundles in one run, or one bundle with a checkpoint after each."
...
alternatives:
  ...
  - option: "Open a pull request per bundle onto the default branch"
    rejected_because: "It multiplies the reviews the path exists to collapse. It also puts a partially applied path in front of everyone before the reviewer has seen the tree."
...
delivers:
  capability: "A team accepts a whole merge path in one decision. It still stops after any bundle to check out the resulting tree before releasing the next."
...
invalidates it. The tree the runner produces can differ from the tree the replay
produced. The first merge is where that shows up.

**A checkpoint has to be a real tree.** The reviewer's stated preference is to
commit to a path once and then control the pace. They merge one bundle and read
the result before releasing the next. That preference is only meaningful if the
intermediate state sits somewhere they can check out.

The worked scenario gives the shape. Ten pull requests fall into three bundles.
Five finish the single plugin. One splits the repository into five plugins. Four
name and fix the new structure.

## Options considered

1. **Merge each bundle onto the default branch.** Simplest to implement. It makes
   bundle one irreversible before bundle two is attempted.
...
A runner reads an accepted path from the ledger. It merges each bundle onto its
...
bundle at a time is the default. The runner merges a bundle and stops, and the
...
Out of scope: merging the final pull request. That stays a human action on the
...
- **Positive:** every checkpoint is a real tree with a name. Reading the
...
- **Negative:** the integration branches drift from the default branch while the
  ...
  ordered to avoid.
- **Negative:** this is the first remote write in the plugin family. The adopting
  repository supplies the runner rather than the plugin, so its credentials and
  its permissions sit outside this design.
