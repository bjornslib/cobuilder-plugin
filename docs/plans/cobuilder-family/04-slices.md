...
| 4 | `plugin-split/E1` | Renames inside today's single plugin | Six architecture modes, `explore-design` deleted, `submit` renamed to `generate`, and the old `generate` renamed to `review` |
| 5 | `plugin-split/E1` | The 33 cross-pillar references are fixed | The grep regression test passes while the repo is still one plugin |
| 6 | `plugin-split/E1` | Five plugins, five manifests | `claude plugin validate` passes on each, and the family installs |
| 7 | `plugin-split/E1` | The two ports land | `cobuilder-implement` renamed and shipped, `collaborate-with-user` folded in, the orientation skill written |
...
| 1 | `plugin-split/E2` | **Tracer bullet: two plugins, one shared module** | Two throwaway plugins install from a local marketplace and both resolve the shared module from their own cache |
...
| 2 | `plugin-split/E3` | The compatibility gate exists | `require_compatible()` and `stamp_generator()` pass their tests, called by nothing yet |
| 3 | `plugin-split/E3` | The gate is wired and the bundle migrates | Format 3, `min_reader_schema`, the generators map, every writer gated |
...
| 8 | `plugin-split/E4` | The index holds the entities | `build_index.py` emits every entity, and the two scripts it replaces are gone |
| 9 | `plugin-split/E4` | The index resolves the joins | `adr_to_pull_request`, `slice_to_epic`, `district_uncovered`, and freshness |
...
| 10 | `plugin-split/E5` | The Decisions and Contexts modes | A person browses every decision and every context, with the anchor distinction visible |
| 11 | `plugin-split/E5` | The Builds mode and the Backlog lane | Gates driven by the status document, the Backlog lane computed from the index, pages moved into the bundle |
...
| 12 | `plugin-split/E6` | The ledger and its projection | Append, fold, and project, with no server involved |
| 13 | `plugin-split/E6` | The anchor and the write endpoint | Click to anchor a sentence, `POST /feedback` appends, the drawer keeps the text in view |
| 14 | `plugin-split/E6` | The wake command and the whole loop | The background command blocks and returns new threads, and the loop runs once end to end |
