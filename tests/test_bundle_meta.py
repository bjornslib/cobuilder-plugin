...
def test_stamp_generator_preserves_other_plugin(tmp_path):
    write_bundle_json(
        tmp_path, {"generators": {"cobuilder-artifact": "0.5.0"}}
    )
...
def test_stamp_generator_overwrites_own_prior_entry(tmp_path):
    write_bundle_json(tmp_path, {"generators": {"cobuilder-pr": "0.1.0"}})
    stamp_generator(tmp_path, "cobuilder-pr", "0.2.0")

    bundle_meta = json.loads((tmp_path / "bundle.json").read_text())
    assert bundle_meta["generators"] == {"cobuilder-pr": "0.2.0"}
