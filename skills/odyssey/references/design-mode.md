...
drafts an ADR. Submit mode later joins the design to the pull request that
implements it.

A design never enters `data/story.json`. That file keys on an integer `pr`.
The design enters the timeline only when submit mode files it under the
...
`interview-guide.md` §2.

## 5. Stage 2 — Interview

Five topics only. They fit the six-topic cap in `interview-guide.md` §3.
...
`interview-guide.md` §2. Rank the gaps. Drop a topic the ground step
already closed.
...
from stage 1. Compare the two accounts the way `interview-guide.md` §3a
...
as mermaid authoring and `decision-records-lite.md`:
...
Invert `review-mode.md` §2. There is no diff yet, so there is no line to
...
Answer the same three questions as `review-mode.md` §3, against the
...
written. `verdict` uses the same values as `review-mode.md` §8: `sound`,
...
2. **Diagrams.** Reuse the diagram contract in `diagram-mode.md` for
...
4. **Intent and assessment.** Write `intent.json` and a
...
names. **Do not decompose the work.** Decomposition is cobuilder-factory
G1 work.
...
Submit mode, when it lands, will strip `design/`, take the first segment
as the design name, and take the rest as the epic slug. If that parse
fails, it will scan every `goal.json` for a matching `epics[].branch`.
