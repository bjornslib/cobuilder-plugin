...
- A system instruction that forbids evaluation, ranking, and hedging within the branch. The branch generates. It does not judge itself. Include ADHD's rule explicitly: ban the first three answers everyone would independently reach. If the branch's first instinct is one of them, discard it and go one level deeper into the frame.

Isolation is mechanical, not requested. It comes from launching N separate `Agent` calls with no shared transcript. Never ask one agent to "consider N perspectives" inside a single response. That keeps the anchoring the exercise is designed to prevent. Branches never see each other's output. No branch learns what the other branches produced.
...
| The error is a lie | "The reported symptom is where the failure became visible, not necessarily where it originated. What upstream fault would produce exactly this downstream symptom?" | Misattributed root cause: a stack trace or error message that points at the victim, not the culprit |
...
Issue a SEPARATE critic call after all N diverge branches return, with the opposing system prompt. This call's job is to evaluate, not generate. Hard wall: the critic must not be the same call, session, or context that generated any candidate. It receives all N outputs at once. Leave outputs unlabeled by frame origin if you want to reduce halo effect, though in practice frame labels help the `frame` field in §5, and can stay attached.
...
3. **Cluster** candidates by underlying angle, not by frame (two frames can converge on the same underlying idea), and record `agreement` as the count of independently-run frames that reached that cluster. Agreement is a SECONDARY signal only. Never use it as a filter, a ranking key, or a reason to drop a candidate. That is exactly the parallel-solutioning failure mode this reference corrects. Majority voting synthesizes toward the center and throws away the minority insight the divergence was run to surface. Agreement exists only so a downstream consumer who explicitly wants a consensus view has one available. It must never gate what gets reported.
...
5. **Deepen the top 3** into fuller sketches, chosen by a combination of score and starred status, not by agreement. Give each one its load-bearing risks spelled out and a first concrete step small enough to start immediately.
...
Run H3's test first. It has the lowest cost, and its outcome also informs H1. Test H2 last, not because it is least likely, but because ruling it in or out is the most expensive action available. By the time H1 and H3 are settled, the remaining hypothesis space may already be down to H2 alone. That makes the expensive test unambiguous instead of speculative.

## 5. Output schema

One JSON object per candidate, whether it originated in design, review, or debug mode. In review mode, deliberately shape each candidate with `id`, `severity`, `file`, `line`, `evidence`, and `recommendation` fields. This lets a standard findings array fold in blind-spot output from a review-mode divergence mechanically, without an LLM retyping it into a different shape.
...
Field notes:
- `trap` is boolean. `trap_reasoning` is required whenever `trap` is true, and it should still hold a brief note when `trap` is false, stating what was checked.
...
Total: 5-7 agent calls per divergence run. §1's gate is the primary cost control. It decides whether this run happens at all. Once past the gate, do not trim the frame count as a cost-saving measure. A partial frame set reintroduces the anchoring and blind-spot risk the full catalogue exists to cover. Trim by skipping the whole exercise (§1), not by running fewer frames within it.
