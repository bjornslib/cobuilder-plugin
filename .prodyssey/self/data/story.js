    ...
    districts: [
      {
        ...
        files: 25,
        blurb:
          "Hub-local scratch for centrally-stored bundles and view-server bookkeeping (per-repo-slug subfolders, active symlink, view-server pid/log) — currently holds test-run bundles from prior sessions committed to git rather than gitignored.",
        root_paths: [".prodyssey"],
      },
      {
        id: "commands",
        ...
        files: 4,
        blurb:
          "Thin dispatchers (baseline.md, generate.md, view.md) that forward arguments straight into the odyssey skill — no logic of their own.",
        root_paths: ["commands"],
      },
      {
        id: "scripts",
        ...
        files: 9,
        blurb:
          "PEP 723 uv scripts doing the mechanical data movement the skill orchestrates: extract_story, extract_diffs, generate_prompts, generate_audio, verify_bundle.",
        root_paths: ["scripts"],
      },
      {
        id: "skills",
        ...
        files: 10,
        blurb:
          "SKILL.md orchestration procedure plus on-demand reference docs (story-mode, decision-records-lite, baseline-derivation, adr-template, per-stack detection cards) that hold all the judgment-shaped guidance.",
        root_paths: ["skills"],
      },
    ],
  },
  timeline: [
    {
      ...
      tagline:
        "This PR proposes an orientation doc, plus a full pipeline that turns any generated PR narrative into a shareable Claude Artifact.",
      depth: "detailed",
      size: {
        files: 29,
        adds: 7586,
      ...
      levels: {
        landscape: {
          narration:
            "This PR is now much bigger than it started: 29 files, over 7,500 lines added. It still adds the orientation doc, but the bulk of it is a new pipeline for publishing PR stories as shareable pages.",
          voice:
            "This PR has grown well past where it started. Twenty-nine files, over seventy-five hundred lines added. It still adds the orientation doc, but most of the change is a new pipeline for publishing PR stories as shareable pages.",
        },
        problem_solution: {
          problem:
            'prodyssey had no in-repo orientation document (the original gap this PR set out to close), and separately, no way to share a generated PR story outside a local checkout — the bundle viewer only works served from a real `.odyssey/` directory with its sibling `data/`/`assets/` folders present, so "send someone the story" meant "send someone the repo."',
          solution:
            "`CLAUDE.md` covers layout, the generation flow, and the bundle shape. Alongside it, three new scripts (`export_artifact.py`, `export_index.py`, `record_publish.py`) and a new Publish mode in `SKILL.md` turn a generated PR into a self-contained HTML file — story, ADRs, diff, scene art, and narration all inlined — that Claude publishes directly as a Claude Artifact via `/prodyssey:publish`, plus an index artifact that stays current across every publish run.",
          narration:
            "This PR still writes down how prodyssey fits together, so a session doesn't reconstruct that from scratch. It also solves a second, related problem: a generated PR story only worked if you had the repo checked out and a local server running. Now one command turns that story into a link anyone can open.",
          beats: [
            {
              kind: "background",
          ...
          voice:
            "This proposes writing down how prodyssey fits together, so a session doesn't reconstruct that from scratch. It also solves a second problem: a generated PR story only worked if you had the repo checked out and a local server running. Now one command turns that story into a link anyone can open.",
        },
        architecture: {
          narration:
            "Two decisions here: how to make the viewer artifact-safe at all, and how to know when a published artifact needs updating.",
          voice:
            "Two decisions here: how to make the viewer artifact-safe at all, and how to know when a published artifact needs updating.",
          beats: [
            {
        ...
        file_changes: {
          narration:
            "29 files: the orientation doc, the three new export scripts and their skill/command wiring, a couple of pre-existing scripts touched for open-PR/commit support, and the regenerated bundle output — including this PR's own just-published artifacts — for both the self-analysis bundle and one other repo's centrally-stored bundle.",
          groups: [
            {
